/* perf_stat.h
 * copyrights hoddis<yutao_2006@126.com> 2015
**/
...
struct perf_entry {
  ...
  unsigned long long pe_cost_total;
...
public:
  ...
  _oStream& print_header(_oStream& out) const {
    out << "Name\tTotal-Called\tTotal-Cost\tAvg-Cost\tMin-Cost\tMax-Cost"
      << std::endl;
    return out;
  ...
  _oStream& print(_oStream& out) {
    pthread_mutex_lock(&ps_lock);
    for (const_iterator iter = ps_stat_map.begin(); iter != ps_stat_map.end();
        ++iter) {
...
class perf_guard {
public:
  perf_guard(const char *name, perf_stat& state = perf_stat::instance())
      : pg_name(name), pg_state(state) {
...
#define TRACE_PROF() newstone::perf_guard __trace_prof(__PRETTY_FUNCTION__)
#define TRACE_PROF_NAME(name) newstone::perf_guard  __trace_prof_name(name)
#else
#define TRACE_PROF()  do {} while (0);
#define TRACE_PROF_NAME(name) do {} while(0);
